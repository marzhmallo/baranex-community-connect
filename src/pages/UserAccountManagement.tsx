...
const UserAccountManagement = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('profiles').select('*').eq('brgyid', userProfile.brgyid).order('created_at', {
        ascending: false
      });
      if (error) {
  ...
  const filteredUsers = users?.filter(user => {
    ...
    const matchesTab = activeTab === 'all' || activeTab === 'admins' && (user.role === 'admin' || user.role === 'staff') || activeTab === 'users' && user.role === 'user' || activeTab === 'pending' && user.status === 'pending' || activeTab === 'approved' && user.status === 'approved' || activeTab === 'blocked' && user.status === 'blocked';
                                  ...
                                  {selectedUser && <div className="space-y-4">
                                      ...
                                      <div className="flex space-x-2">
                                        {selectedUser.status === 'pending' && <>
                                            <Button size="sm" onClick={() => updateUserStatus(selectedUser.id, 'approved')}>
                                              <Check className="h-4 w-4 mr-1" />
                                              Approve
                                            </Button>
                                            <Button size="sm" variant="destructive" onClick={() => updateUserStatus(selectedUser.id, 'rejected')}>
