...
const CreateIncidentDialog = ({ open, onOpenChange }: CreateIncidentDialogProps) => {
  ...
  // Load residents when dialog opens
  const handleOpenChange = (isOpen: boolean) => {
    if (isOpen) {
      fetchResidents();
    } else {
      // Reset form and parties when dialog closes
      form.reset();
      setParties([]);
  ...
  const handleAddParty = () => {
    ...
    setParties(prev => [...prev, partyData]);
    setNewParty({ role: '', resident_id: '', name: '', contact_info: '' });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Then, add all parties to the incident
      if (parties.length > 0) {
        ...
        if (partiesError) {
          ...
          toast({
            title: "Warning",
            description: "Incident created but failed to add some parties",
            variant: "destructive",
          });
        }
      }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new crime incident report for your barangay and add involved parties
          </DialogDescription>
        </DialogHeader>
                    ...
                    {complainants.map((party, index) => (
                      <div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          size="sm"
                          onClick={() => handleRemoveParty(parties.findIndex(p => p === party))}
                        >
