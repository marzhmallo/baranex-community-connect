...
export const NotificationsPage = () => {
  ...
  const renderNotificationCard = (notification: Notification) => {
    const content = (
      <div className={cn(
        "group relative p-6 border border-border/40 rounded-xl transition-all duration-300 cursor-pointer",
        "hover:border-border hover:shadow-lg hover:shadow-primary/5",
        !notification.read 
          ? "bg-gradient-to-r from-primary/5 via-background to-background border-primary/20" 
          : "bg-background hover:bg-accent/30"
      )}>
        {/* Unread indicator */}
        {!notification.read && (
          <div className="absolute left-3 top-6 w-2 h-2 bg-primary rounded-full animate-pulse" />
        )}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Badge 
                  variant="secondary" 
                  className="text-xs font-medium bg-accent/80 text-accent-foreground border-0"
                >
                ...
                <Badge 
                  ...
                  className={cn(
                    "text-xs border-0",
                    notification.priority === 'urgent' && "bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300",
                    notification.priority === 'high' && "bg-orange-50 text-orange-700 dark:bg-orange-950 dark:text-orange-300",
                    notification.priority === 'normal' && "bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300",
                    notification.priority === 'low' && "bg-gray-50 text-gray-700 dark:bg-gray-950 dark:text-gray-300"
                  )}
                >
    ...
    if (notification.type === 'dnexus' || notification.type === 'dnexus_status') {
      return (
        <Link to="/nexus" onClick={() => handleNotificationClick(notification)} key={notification.id}>
          {content}
        </Link>
      );
    } else if (notification.linkurl) {
      return (
        <Link to={String(notification.linkurl)} onClick={() => handleNotificationClick(notification)} key={notification.id}>
          {content}
        </Link>
      );
    } else {
      return (
        <div onClick={() => handleNotificationClick(notification)} key={notification.id}>
          {content}
        </div>
      );
    }
          ...
          {hasMore && notifications.length > 0 && (
            <div className="flex justify-center pt-6">
              <Button 
                ...
                variant="outline"
                disabled={loading}
                className="min-w-[200px] border-border/60 hover:bg-accent/50"
              >
                {loading ? (
                  <>
