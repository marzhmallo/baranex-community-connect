...
const ThreadsView = ({
  ...
  const fetchThreads = async () => {
    try {
      let query = supabase.from('threads').select('*').eq('forum_id', forum.id);
      ...
      const {
        data: profilesData,
        error: profilesError
      } = await supabase.from('profiles').select('id, firstname, lastname, profile_picture').in('id', userIds);
      ...
      const avatarEntries = await Promise.all(profilesData.map(async (user: any) => {
      ...
      await Promise.all(avatarUrls.map(src => new Promise<void>(resolve => {
        const img = new Image();
        img.onload = () => resolve();
        img.onerror = () => resolve();
        img.src = src;
      ...
      const commentData = await Promise.all(threadsData.map(async thread => {
        const {
          count
        ...
        const {
          data: lastComment
        } = await supabase.from('comments').select('created_at').eq('thread_id', thread.id).order('created_at', {
          ascending: false
        }).limit(1).maybeSingle();
        return {
          threadId: thread.id,
      ...
      const reactionData = await Promise.all(threadsData.map(async thread => {
        const {
          count
        ...
        if (userProfile) {
          ...
          } = await supabase.from('reactions').select('emoji').eq('thread_id', thread.id).eq('user_id', userProfile.id).maybeSingle();
  ...
  const handleDeleteForum = async () => {
    ...
    try {
      ...
      const {
        data: forumThreads
      } = await supabase.from('threads').select('id').eq('forum_id', forum.id);
      if (forumThreads) {
      ...
      const {
        error
      } = await supabase.from('forums').delete().eq('id', forum.id);
      if (error) throw error;
      toast({
        title: "Success",
  ...
  if (selectedThread) {
    return <ThreadDetailView thread={selectedThread} onBack={handleBackToThreads} isUserFromSameBarangay={isUserFromSameBarangay} isPublicForum={forum.is_public} />;
  }
  ...
  return <div className="w-full mx-auto p-6 bg-background min-h-screen relative">
      ...
      <div className="mb-8">
        <Button variant="ghost" className="w-fit flex items-center mb-4" onClick={onBack}>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {userProfile}
            {userProfile?.id === forum.created_by && <div className="relative">
                <Button variant="outline" size="sm" onClick={() => setShowForumActions(!showForumActions)} className="p-2">
                  <MoreHorizontal className="h-4 w-4" />
                </Button>
                {showForumActions && <div className="absolute right-0 top-full mt-1 bg-background border border-border rounded-lg shadow-lg z-50 w-48">
                    <div className="py-1">
                      <button onClick={() => {
                  ...
                  setShowForumActions(false);
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <button onClick={() => setShowForumActions(false)} className="w-full px-4 py-2 text-left hover:bg-muted transition-colors duration-200 flex items-center gap-2 text-sm text-destructive">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search threads..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-background text-foreground border border-input rounded-lg focus:ring-2 focus:ring-ring focus:border-transparent outline-none transition-all duration-200 placeholder:text-muted-foreground" />
          </div>
          
          <div className="flex gap-2">
            <div className="relative">
              <Button variant="outline" className="px-4 py-3 flex items-center gap-2" onClick={() => setShowSortDropdown(!showSortDropdown)}>
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {showSortDropdown && <div className="absolute right-0 top-full mt-2 bg-background border border-border rounded-lg shadow-lg z-50 w-48">
                  <div className="py-2">
                    <button className="w-full px-4 py-3 text-left hover:bg-muted transition-colors duration-200 flex items-center gap-2" onClick={() => {
                  setSortOrder('newest');
            ...
            <div className="relative">
              <Button variant="outline" className="px-4 py-3 flex items-center gap-2" onClick={() => setShowFilterDropdown(!showFilterDropdown)}>
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {showFilterDropdown && <div className="absolute right-0 top-full mt-2 bg-background border border-border rounded-lg shadow-lg z-50 w-64 p-4">
                  <div className="space-y-3">
                    <div>
                      <h4 className="font-medium text-foreground mb-2">Status</h4>
                      <div className="flex flex-wrap gap-2">
                        <button className={`px-2 py-1 text-xs border rounded-full transition-colors ${statusFilter === 'all' ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-muted'}`} onClick={() => setStatusFilter('all')}>
                          All
                        </button>
                        <button className={`px-2 py-1 text-xs border rounded-full transition-colors ${statusFilter === 'pinned' ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-muted'}`} onClick={() => setStatusFilter('pinned')}>
                          Pinned
                        </button>
                        <button className={`px-2 py-1 text-xs border rounded-full transition-colors ${statusFilter === 'locked' ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-muted'}`} onClick={() => setStatusFilter('locked')}>
                          Locked
                        </button>
                        <button className={`px-2 py-1 text-xs border rounded-full transition-colors ${statusFilter === 'open' ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-muted'}`} onClick={() => setStatusFilter('open')}>
                          Open
                        </button>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex flex-col gap-1">
                        <button className={`w-full px-3 py-2 text-left text-sm rounded transition-colors ${dateFilter === 'today' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`} onClick={() => setDateFilter('today')}>
                          Today
                        </button>
                        <button className={`w-full px-3 py-2 text-left text-sm rounded transition-colors ${dateFilter === 'week' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`} onClick={() => setDateFilter('week')}>
                          This week
                        </button>
                        <button className={`w-full px-3 py-2 text-left text-sm rounded transition-colors ${dateFilter === 'month' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`} onClick={() => setDateFilter('month')}>
                          This month
                        </button>
                        <button className={`w-full px-3 py-2 text-left text-sm rounded transition-colors ${dateFilter === 'all' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`} onClick={() => setDateFilter('all')}>
                          All time
                        </button>
                      </div>
                    </div>
        ...
        <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-muted border border-border rounded-lg mb-6">
          ...
          <Button variant="outline" size="sm" className={`px-3 py-1 rounded-full text-sm transition-colors duration-200 flex items-center gap-1 ${statusFilter === 'pinned' ? 'bg-primary text-primary-foreground' : 'bg-background hover:bg-muted'}`} onClick={() => setStatusFilter(statusFilter === 'pinned' ? 'all' : 'pinned')}>
            <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
            </svg>
            Pinned
          </Button>
          <Button variant="outline" size="sm" className={`px-3 py-1 rounded-full text-sm transition-colors duration-200 flex items-center gap-1 ${sortOrder === 'recent_activity' ? 'bg-primary text-primary-foreground' : 'bg-background hover:bg-muted'}`} onClick={() => setSortOrder(sortOrder === 'recent_activity' ? 'newest' : 'recent_activity')}>
            <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Recent
          </Button>
          <Button variant="outline" size="sm" className={`px-3 py-1 rounded-full text-sm transition-colors duration-200 flex items-center gap-1 ${sortOrder === 'popular' ? 'bg-primary text-primary-foreground' : 'bg-background hover:bg-muted'}`} onClick={() => setSortOrder(sortOrder === 'popular' ? 'newest' : 'popular')}>
            <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
            </svg>
            Popular
      ...
      <div className="space-y-4">
        ...
        <ThreadList threads={filteredThreads} onThreadSelect={handleThreadSelected} onPinToggle={handlePinToggle} onLockToggle={handleLockToggle} canModerate={userProfile?.id === forum.created_by} onEditThread={handleEditThread} onDeleteThread={handleDeleteThread} />
      </div>

      {showCreateDialog && <CreateThreadDialog open={showCreateDialog} onOpenChange={open => {
      setShowCreateDialog(open);
      if (!open) setEditingThread(null);
    }} onThreadCreated={handleThreadCreated} forum={forum} />}

      {showEditForumDialog && <EditForumDialog open={showEditForumDialog} onOpenChange={setShowEditForumDialog} forum={forum} onForumUpdated={refetch} />}
    </div>;
};
